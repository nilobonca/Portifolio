...
import floatingStyles from "@/components/WhatsAppFloatingButton.module.css";
...
export default function HomeClient({ projects }: { projects: ProjectData[] }) {
  ...
  const [isFloatingWhatsAppVisible, setIsFloatingWhatsAppVisible] = useState(false);

  useEffect(() => {
    const headerBtn = headerBtnRef.current;
    const contactBtn = contactBtnRef.current;

    let isHeaderVisible = true;
    let isContactVisible = false;
    let isScrolledPastHeader = false;

    const checkFloatingState = () => {
      setIsFloatingWhatsAppVisible(isScrolledPastHeader && !isHeaderVisible && !isContactVisible);
    };
    ...
    if (headerBtn) headerObserver.observe(headerBtn);
    if (contactBtn) contactObserver.observe(contactBtn);
    ...
    <div className={styles.container} ref={container}>
      <header className={styles.header}>
        ...
        <div className={styles.headerActions}>
          <a
            ref={headerBtnRef}
            href="https://wa.me/5521986200124"
      ...
      <a
        ...
        target="_blank"
        rel="noopener noreferrer"
        className={`${floatingStyles.floatingButton} ${isFloatingWhatsAppVisible ? floatingStyles.visible : ""}`}
        aria-label="Contato via WhatsApp"
      >
        <WhatsAppIcon size={20} />
        <span className={floatingStyles.floatingText}>WhatsApp</span>
      </a>
